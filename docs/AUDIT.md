...
| Med | honesty | The unqualified "hardware-sealed" headline (README/CLI) contradicted the code (no shipped sealer is hardware-backed) and the product's own honesty model. | Reworded to "OS-sealed (DPAPI/Keychain/passphrase; not a hardware TPM)" on every external surface. |
